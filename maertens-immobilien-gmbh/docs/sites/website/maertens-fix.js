/* Maertens Fix v1 — cookie banner + immo iframe + testimonials + image fallback */
(function () {
  'use strict';
  ...
  function injectCookieBanner() {
    ...
    if (localStorage.getItem('ml-cookie-consent') === 'accepted' ||
        localStorage.getItem('ml-cookie-consent') === 'declined') return;
    ...
    b.innerHTML = ''
      ...
      +   '<a href="datenschutz/" rel="nofollow">Mehr zum Datenschutz</a>.'
      + '</div>'
  ...
  // ─── 2) Immobilien-Embed (only on home page) ─────────────────────────
  function injectImmoEmbed() {
    ...
    // find anchor: existing #immobilien section or before <footer>
    ...
    wrap.innerHTML = ''
      ...
      + 'title="Aktuelle Immobilien Maertens"></iframe>';
    if (anchor === document.querySelector('footer')) {
  ...
  // ─── 3) Testimonials slider fix ──────────────────────────────────────
  function fixTestimonials() {
    if (window.Swiper) return; // real swiper present, leave it alone
    ...
    blocks.forEach(function (block) {
      ...
      slides.forEach(function (s) {
        s.classList.add('ml-tslider-slide');
        track.appendChild(s);
      });
      block.innerHTML = '';
      block.appendChild(track);
      var nav = document.createElement('div');
      nav.className = 'ml-tslider-nav';
  ...
  // ─── 4) Broken image fallback (Über uns etc.) ────────────────────────
  function fixBrokenImages() {
    document.querySelectorAll('img').forEach(function (img) {
      img.addEventListener('error', function () {
        img.classList.add('ml-broken');
        img.alt = img.alt || 'Bild folgt';
      });
      // proactively flag empty src
